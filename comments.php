	...
	<li <?php comment_class(); ?>>
		<article id="comment-<?php comment_ID(); ?>">
			<header class="comment-author vcard">
				...
				<?php edit_comment_link(__('(Rediger)', 'reverie'), '', '') ?>
			</header>
...
<?php if ( have_comments() ) : ?>
	<section id="comments">
		<h3><?php comments_number(__('Ingen kommentarer til', 'reverie'), __('En kommentar til', 'reverie'), __('% kommentarer til', 'reverie') ); ?> &#8220;<?php the_title(); ?>&#8221;</h3>
		...
		<footer>
			<nav id="comments-nav">
				<div class="comments-previous"><?php previous_comments_link( __( '&larr; Se ældre kommentarer', 'reverie' ) ); ?></div>
				<div class="comments-next"><?php next_comments_link( __( 'Se nyere kommentarer &rarr;', 'reverie' ) ); ?></div>
			</nav>
		</footer>
	</section>
...
<?php if ( comments_open() ) : ?>
<section id="respond">
	<h3><?php comment_form_title( __('Skriv en kommentar', 'reverie'), __('Skriv et svar to %s', 'reverie') ); ?></h3>
	<p class="cancel-comment-reply"><?php cancel_comment_reply_link(); ?></p>
	<?php if ( get_option('comment_registration') && !is_user_logged_in() ) : ?>
	<p><?php printf( __('Du skal være logget ind <a href="%s">logged in</a> for at skrive en kommentar.', 'reverie'), wp_login_url( get_permalink() ) ); ?></p>
	<?php else : ?>
	<form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform">
		...
		<?php endif; ?>
		<p>
			<label for="comment"><?php _e('Din kommentar', 'reverie'); ?></label>
			<textarea name="comment" id="comment" tabindex="4"></textarea>
		</p>
		
		<p><input name="submit" class="radius medium button" type="submit" id="submit" tabindex="5" value="<?php _e('Indsend', 'reverie'); ?>"></p>
